...
def preprocess(path, scale=3):
    ...
    bicbuic_img = cv2.resize(label_, None, fx=1.0 / scale, fy=1.0 / scale,
                             interpolation=cv2.INTER_CUBIC)  # Resize by scaling factor
    input_ = cv2.resize(bicbuic_img, None, fx=scale, fy=scale,
                        interpolation=cv2.INTER_CUBIC)  # Resize by scaling factor
...
def load_data(is_train, test_img):
    if is_train:
        data = prepare_data(dataset="datasets")
    else:
        if test_img != "":
